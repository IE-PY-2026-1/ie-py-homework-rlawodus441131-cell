...
## 2. 시나리오 (5~10줄)
* 이 프로그램은 개인 투자자가 자신의 주식/코인 매매 기록을 체계적으로 관리하고 전략의 성과를 분석하는 디지털 매매 비서입니다. 가장 먼저 사용자가 매매한 종목명, 매수가, 매도가, 수량 등의 기본 데이터를 입력하면 실시간 손익과 수익률을 계산하며 여정이 시작됩니다. 단순한 계산을 넘어, 수익률에 따라 매매 등급(S~F)을 부여하여 사용자의 매매 습관을 객관적으로 진단해 줍니다. 특히 특정 수익률을 달성하거나 손절 원칙을 잘 지켰을 때는 '냉철한 승부사' 같은 특별 칭호와 함께 가상의 보너스 포인트를 부여하여 투자 동기를 부여합니다. 나중에는 한 번의 매매가 아니라 수많은 매매 기록을 커다란 주머니(리스트)에 담아 승률과 누적 수익을 한눈에 관리할 수 있게 만들 것입니다. 최종적으로는 메뉴판 시스템을 통해 언제든 기록을 조회하고 통계를 확인할 수 있는 전문적인 퀀트 소프트웨어로 완성될 계획입니다.
